...
let playlistData = async (userArray) => {
  return await apiData(userArray);
};
...
function getUserInputs() {
  // if (document.getElementById("user-input-1").value === "") {
  //   alert("First input is empty");
  // }

  // if (document.getElementById("user-input-2").value === "") {
  //   alert("2nd input is empty");
  // }

  let inputs = document.querySelectorAll(".user-input");
  let arr = Array.from(inputs);
  return arr.filter((el) => el === null).map((el) => el.value);
}

function generateList(dataArr) {
